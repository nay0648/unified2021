...
for fidx = 1 : num_file  
  ...
  %----------------------------------------------------------------------
  x = [];
  for m = 1 : length(ifname)
    %[tmp,FS,NBITS] =  wavread(ifname{m});
    [tmp,FS] =  audioread(ifname{m});
    x = [x, tmp];
  end
  num_mic = length(ifname);
  x = x(:, 1 : num_mic);
  ...
  % Write microphone signals
  %----------------------------------------------------------------------
  for m = 1 : length(ofname)
    %wavwrite(y(:,m),FS,NBITS,ofname{m});
    audiowrite(ofname{m},y(:,m),FS);
  end
end
