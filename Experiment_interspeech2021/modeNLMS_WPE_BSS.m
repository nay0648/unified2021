function dataout=modeNLMS_WPE_BSS(nummics, numrefs, datain)
%
% Perform nlms, wpe, and bss.
...
nearend1=speex_mdf(fs, datain(:, 3), datain(:, 1), filter_length, frame_size);
nearend2=speex_mdf(fs, datain(:, 3), datain(:, 2), filter_length, frame_size);
...
Xtf=cell(M, 1);
for m=1:M
    Xtf{m}=stft(early(:, m), stftshift, fftsize, false);
end
[K, T]=size(Xtf{1});
